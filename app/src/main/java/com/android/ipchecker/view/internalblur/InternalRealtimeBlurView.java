package com.android.ipchecker.view.internalblur;
 ...
 * </ul>
 */
public class InternalRealtimeBlurView extends View {
    private float mDownsampleFactor; // default 4
    private int mOverlayColor; // default #aaffffff
    private float mBlurRadius = 20; // default 10dp (0 < r <= 25)
    ...
    public static final int InternalRealtimeBlurView_internalRealtimeBlurRadius = 0;
    public static final int InternalRealtimeBlurView_internalRealtimeDownsampleFactor = 1;
    public static final int InternalRealtimeBlurView_internalRealtimeOverlayColor = 2;

    public InternalRealtimeBlurView(Context context, AttributeSet attrs) {
        super(context, attrs);
        mBlurImpl = getBlurImpl(); // provide your own by override getBlurImpl()

        int[] items = getResourceDeclareStyleableIntArray(context);
        TypedArray a = context.obtainStyledAttributes(attrs, items);

        mBlurRadius = a.getDimension(InternalRealtimeBlurView_internalRealtimeBlurRadius,
                TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 10, context.getResources().getDisplayMetrics()));
        mDownsampleFactor = a.getFloat(InternalRealtimeBlurView_internalRealtimeDownsampleFactor, 4);
        mOverlayColor = a.getColor(InternalRealtimeBlurView_internalRealtimeOverlayColor, 0xAAFFFFFF);
        a.recycle();

        mPaint = new Paint();
    }

    private static int[] getResourceDeclareStyleableIntArray(Context context) {
        try {
            Field[] fields2 = Class.forName(context.getPackageName() + ".R$styleable").getFields();
            for (Field f : fields2) {
                if (f.getName().equals("InternalRealtimeBlurView")) {
    ...
    protected BlurImpl getBlurImpl() {
        if (getContext().getPackageManager().hasSystemFeature("android.car.blur.by.gpu")) {
            BLUR_IMPL = 0;
        }
        ...
        switch (BLUR_IMPL) {
            case 1:
                return (BlurImpl) new AndroidXBlurImpl();
            case 2:
                return (BlurImpl) new SupportLibraryBlurImpl();
            case 3:
...
//		}
    ...
    protected void onDraw(Canvas canvas) {
        super.onDraw(canvas);
        Log.d("TungNM6_RealtimeBlurView", "onDraw");
